...
### Added
- **Time Window Filtering** - Fix for false positive root cause correlations
  - New `_is_finding_in_time_window()` helper method
  - Upgrade detection now only considers HISTORICAL_EVENT findings within time window
  - Node pressure cascade correlation respects time boundaries
  - Prevents old cluster upgrade events from being detected as recent

- **Parallel kubectl Execution** - Performance optimization
  - New `_batch_kubectl_calls()` method for concurrent kubectl commands
  - `collect_cluster_statistics()` runs 24 kubectl calls in parallel
  - Reduces cluster statistics collection time by ~60%

- **Parallel CloudWatch Insights Queries** - Major performance improvement
  - CloudWatch Logs Insights queries now start in parallel
  - Concurrent polling for all query results
  - Reduces Insights correlation time from ~120s to ~30s

### Fixed
- **Root cause display** - HTML report now shows first issue when no correlations found
  - Better fallback message when correlation patterns don't match
  - Improved root cause summary section

### Changed
- **Version** - Updated to 3.8.0
- **AGENTS.md** - Updated test count to 215
- **README** - Updated version badge to 3.8.0
